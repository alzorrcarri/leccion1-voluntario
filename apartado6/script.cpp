...
int main (void)
{
    ...
    fichero_desviacion.open("/Users/alzorrcarri/Documents/cphys/leccion1-voluntario/desviacion_sinincr.dat");
   ...
   /* Estudiamos la fluctuación de la posición de la partícula número 6*/
   xini=x[5]; yini=y[5];
   ...
   for ( k = 0; k < 30000; k++)
   {
      ...
      fichero_desviacion << t << '\t' << pow(sqrt(x[5]*x[5]+y[5]*y[5])-sqrt(xini*xini+yini*yini),2) << '\n';
      ...
      if ((k*h==20.0) || (k*h==30.0) || (k*h==35.0) || (k*h==45.0))
      {
         for ( i = 0; i < n-1; i++)
         {
            vx[i]=1.5*vx[i];
